...
# This script builds the project and generates Lider (lider-firewall.jar) & Lider Console (lider-console-firewall.jar) distribution files as well as Ahenk package (firewall.deb)
#
# Generated files can be found under /tmp/lider-ahenk-firewall-plugin/
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-firewall
dpkg-buildpackage -b -uc
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-firewall-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-firewall/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-firewall/target/lider-console-*.jar "$EXPORT_PATH"
